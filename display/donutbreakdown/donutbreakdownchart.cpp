    ...
    for (QAbstractSeries *series : allseries) {
        ...
        for (QLegendMarker *marker : markers) {
            ...
            if (series == m_mainSeries) {
                ...
                for(const auto &v : m_mainSeries->slices() )
                {
//                    qDebug() << v->label();
                    QStringList list = v->label().split(" ");

                    if(series->name() == list.at(0))
                    {
                        mPercentage = list[1].replace(list[1].indexOf('%'), 1, ' ').toDouble();
//                        qDebug() << "0 " << list.at(0) << "1 "
//                                 << mPercentage;
                ...
                pieMarker->setLabel(QString("%1 %2%")
                                    ...
                                    .arg(pieMarker->slice()->percentage() * mPercentage, 0, 'f', 2));
...
void DonutBreakdownChart::refresh(void)
{
